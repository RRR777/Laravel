...
@section('content')
    ...
    <form>
       {{--   <button onclick="location.href='{{url('radars')}}'" type="button">Automobiliai</button>  --}}
        <button onclick="location.href='{{url('drivers')}}'" type="button" class="btn btn-info">{{ __('buttons.drivers') }}</button>
        <button onclick="location.href='{{url('radars/create')}}'" type="button" class="btn btn-info">{{ __('buttons.new_radar') }}</button><br>
    </form><br>
